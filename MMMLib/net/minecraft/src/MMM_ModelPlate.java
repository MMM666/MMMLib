...
public class MMM_ModelPlate extends ModelBox {
    ...
    public MMM_ModelPlate(ModelRenderer modelrenderer, int pTextureX, int pTextureY, float f, float f1, float f2,
    		int pWidth, int pHeight, int pPlane, float pZoom) {
    	// 親は呼んでるけど、内容は未使用
...
//        f6 += f3;
    	...
    	if ((pPlane & 4) > 0) {
    		...
    		quadList[0] = new TexturedQuad(new PositionTextureVertex[] {
                ...
                }, pTextureX, pTextureY, pTextureX + pWidth, pTextureY + pHeight, modelrenderer.textureWidth, modelrenderer.textureHeight);
        } else {
        	quadList[0] = new TexturedQuad(new PositionTextureVertex[] {
