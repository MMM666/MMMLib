...
public class EntityCapsBase implements IModelCaps {
...
//		caps.put("PosBlock", caps_PosBlock);
	...
	public EntityCapsBase(EntityLivingBase pOwner) {
		owner = pOwner;
	}
	...
	public Object getCapsValue(int pIndex, Object... pArg) {
		...
		case caps_Entity:
			return owner;
		case caps_health:
			return (int)Math.ceil(owner.getHealth());
		...
		case caps_isDead:
			return owner.isDead;
		case caps_isJumping:
			return ReflectionHelper.getPrivateValue(EntityLivingBase.class, owner, "isJumping", "field_70703_bu");
//			return owner.isJumping;
		case caps_isInWeb:
			return ReflectionHelper.getPrivateValue(EntityLivingBase.class, owner, "isInWeb", "field_70134_J");
...
//			return owner.isBlocking();
		...
		case caps_PosBlockID:
			return Block.getIdFromBlock(owner.worldObj.getBlock(
					MathHelper.floor_double(owner.posX + (Double)pArg[0]),
					MathHelper.floor_double(owner.posY + (Double)pArg[1]),
					MathHelper.floor_double(owner.posZ + (Double)pArg[2])));
